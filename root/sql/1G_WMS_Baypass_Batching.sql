...
and z.zone = 'GROUND'
and tcp.created_date::date between '2025-01-01' and '2025-02-28'       -- adjust dates and use wider date range
group by 1,2,3,4,5
...
, flexsim_input as (
select 
       ((date_rank - 1) * 24) + batch_create_hour AS continuous_hour,
       (((date_rank - 1) * 86400) + batch_create_seconds) - 25200.00 AS continuous_seconds, -- change 25200 based on the starting hour of batch_create_hour
       ...
       aisle_sequence    
from PreQ
where batch_rule_name <> 'Singles - PK Reinduct'
order by 1,2,3,8
)

select
        continuous_seconds as flexsim_time,
        batch_id,
        containerID,
        item_number,
        tran_qty,

...
class DataValidator:
    ...
    and generates summary metrics for container and slotbook data.
    """
    ...
    def validate(self, container_data: pd.DataFrame, 
        ...
        # Limit container data to first 100 rows
        #container_data = container_data.sort_values(['container_id', 'arrive_datetime']).head(5000)  #Todo - for testing purposes only, comment out later 
    ...
    def _validate_business_rules(self, container_data: pd.DataFrame, slotbook_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Drop duplicates keeping first occurrence (highest qty due to sort)
        ...
        for item, required, available in insufficient_items:
            ...
            additional_qty = required - available + 1000  # Add buffer of 100

            # Adjust inventory
            slotbook_data.loc[index_to_adjust, ['actual_qty', 'altered']] = [slotbook_data.at[index_to_adjust, 'actual_qty'] + additional_qty, 1]
    ...
    def _generate_summary_metrics(self, container_data: pd.DataFrame, slotbook_data: pd.DataFrame):
        """Generate and display summary metrics"""
        ...
        inventory_metrics = [
            ["Total SKUs in slotbook", slotbook_data['item_number'].nunique()],
            ["Total locations", slotbook_data['location_id'].nunique()],
            ["Average quantity per location", 
             round(slotbook_data['actual_qty'].mean(), 2)]
        ]
        
        self.logger.info("Inventory Metrics:")
        self.logger.info("\n" + tabulate(inventory_metrics, 

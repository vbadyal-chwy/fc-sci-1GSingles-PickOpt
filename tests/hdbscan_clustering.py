...
def determine_optimal_clusters(feature_matrix: np.ndarray, 
    ...
    """
    Determine optimal number of clusters using HDBSCAN with different min_cluster_size values
    ...
    logger.info(f"Testing HDBSCAN with min_cluster_size from {min_clusters} to {max_clusters}")
    
    # Calculate silhouette scores for different min_cluster_size values
    silhouette_scores = {}
    cluster_counts = {}
    
    # Test different min_cluster_size values (corresponds to min number of points in a cluster)
    for min_cluster_size in range(min_clusters, max_clusters + 1):
        start_time = time.time()
        
        # Create and fit HDBSCAN
        clusterer = hdbscan.HDBSCAN(min_cluster_size=min_cluster_size,                  #bookmark
                                    min_samples=2,
        ...
        cluster_counts[min_cluster_size] = num_clusters
        
        # Ensure we have at least 2 clusters (silhouette score requires at least 2)
        if num_clusters < 2:
            #logger.warning(f"Only found {num_clusters} clusters for min_cluster_size={min_cluster_size}, skipping")
            continue
        
        # Remove noise points for silhouette calculation
        non_noise_mask = cluster_labels >= 0
        if sum(non_noise_mask) <= 1:
            logger.warning(f"Too many noise points for min_cluster_size={min_cluster_size}, skipping")
            continue
            
        # Calculate silhouette score (excluding noise points)
        score = silhouette_score(scaled_features[non_noise_mask], 
                                cluster_labels[non_noise_mask])
        silhouette_scores[min_cluster_size] = score
        
        noise_percentage = (cluster_labels == -1).sum() / len(cluster_labels) * 100
        
        logger.info(f"Min cluster size: {min_cluster_size}, Clusters: {num_clusters}, "
                   f"Noise: {noise_percentage:.1f}%, Silhouette Score: {score:.4f}, "
    ...
    optimal_min_size = max(silhouette_scores.items(), key=lambda x: x[1])[0]
    optimal_clusters = cluster_counts[optimal_min_size]
    
    logger.info(f"Optimal min_cluster_size: {optimal_min_size} produces {optimal_clusters} clusters "
               f"with score {silhouette_scores[optimal_min_size]:.4f}")
    
    return optimal_min_size, silhouette_scores
...
def hierarchical_sub_clustering(container_data: pd.DataFrame, 
    ...
    """
    Apply HDBSCAN sub-clustering to a set of containers
    ...
    # Calculate suggested minimum points per cluster based on the data size
    estimated_min_points = max(min_clusters, len(valid_containers) // max_cluster_size)
    max_possible_min_size = min(max_clusters, len(valid_containers) // 2)  # Avoid too many clusters
    
    # Use HDBSCAN with optimal parameters
    optimal_min_size, _ = determine_optimal_clusters(
        feature_matrix, 
        min_clusters=min(5, min_clusters),  
        max_clusters=min(max_possible_min_size, estimated_min_points + 2)
    )
    
    # Standardize features for HDBSCAN
    scaler = StandardScaler()
    scaled_features = scaler.fit_transform(feature_matrix)
    
    # Perform HDBSCAN clustering with optimal parameters
    clusterer = hdbscan.HDBSCAN(                                                                      #bookmark
    ...
    # Map containers to sub-clusters (handling noise points as individual clusters)
    sub_clusters = {}
    
    # Track noise points to create individual clusters
    noise_points = []
    
    for i, container_id in enumerate(valid_containers):
        sub_cluster_id = int(cluster_labels[i])
        # Collect noise points (-1) to handle individually
        if sub_cluster_id == -1:
            noise_points.append(container_id)
        else:
            # Regular cluster points
            cluster_key = f"{parent_cluster_id}_{sub_cluster_id}"
            if cluster_key not in sub_clusters:
                sub_clusters[cluster_key] = []
            sub_clusters[cluster_key].append(container_id)
    
    # Create individual clusters for each noise point
    for i, container_id in enumerate(noise_points):
        noise_cluster_id = f"{parent_cluster_id}_noise_{i}"
        sub_clusters[noise_cluster_id] = [container_id]
    
    if noise_points:
        logger.info(f"{indent}Created {len(noise_points)} individual clusters for noise points at depth {depth}")
    ...
    if generate_visuals and depth < 3:  # Limit visualization depth
        ...
        try:
            generate_hdbscan_visualizations(
                feature_matrix, cluster_labels, container_features, 
                valid_containers, clusterer, sub_cluster_path, use_distinct_aisles,
    ...
    for sub_id, containers in sub_clusters.items():
        # If the sub-cluster is still too large, recurse further
...
def cluster_containers(container_data: pd.DataFrame, slotbook_data: pd.DataFrame, 
                      ...
                      output_path: str = './cluster_analysis') -> Dict[str, List[str]]:
    """
    Cluster containers by aisle requirements with HDBSCAN
    ...
    Dict[str, List[str]]
        Dictionary mapping cluster IDs to lists of container IDs
    """
    logger.info("Clustering containers with HDBSCAN")
    ...
    # Determine optimal parameters for HDBSCAN
    if batch_size is None:
        optimal_min_size, silhouette_scores = determine_optimal_clusters(
            feature_matrix, min_clusters, max_clusters
        )
    else:
        # If batch_size is provided, calculate target min_cluster_size
        target_cluster_size = batch_size * containers_per_tour
        optimal_min_size = max(min_clusters, len(valid_containers) // target_cluster_size)
        logger.info(f"Using batch_size parameter to set min_cluster_size to: {optimal_min_size}")
    ...
    # Map containers to initial top-level clusters (handling noise points)
    initial_clusters = {}
    
    # Handle noise points as separate individual clusters (no batching)
    noise_mask = cluster_labels == -1
    if np.any(noise_mask):
        noise_containers = [valid_containers[i] for i, is_noise in enumerate(noise_mask) if is_noise]
        logger.info(f"Found {len(noise_containers)} noise points ({len(noise_containers)/len(valid_containers)*100:.1f}%)")
        
        # Create individual clusters for each noise point to prevent batching them together
        for i, container_id in enumerate(noise_containers):
            initial_clusters[f"noise_{i}"] = [container_id]
        
        logger.info(f"Created {len(noise_containers)} individual clusters for noise points")
    
    # Handle regular clusters
    for cluster_id in sorted(set(cluster_labels[cluster_labels >= 0])):
        cluster_mask = cluster_labels == cluster_id
        cluster_containers = [valid_containers[i] for i, in_cluster in enumerate(cluster_mask) if in_cluster]
        initial_clusters[str(cluster_id)] = cluster_containers
    ...
    if generate_visuals:
        try:
            generate_hdbscan_visualizations(
                feature_matrix, cluster_labels, container_features, 
                valid_containers, clusterer, output_path, use_distinct_aisles,
                title_prefix="Top Level Clustering"
            )
        except Exception as e:
            logger.warning(f"Failed to generate cluster visualizations: {str(e)}")
    
    # Apply sub-clustering to any oversized clusters
    final_clusters = {}
    ...
    if missing_containers:
        ...
        # Create individual clusters for missing containers (same as noise points)
        for i, container_id in enumerate(missing_containers):
            sorted_clusters[f"missing_{i}"] = [container_id]
        
        logger.info(f"Created {len(missing_containers)} individual clusters for containers with missing data")
...
def generate_hdbscan_visualizations(feature_matrix: np.ndarray, 
    ...
    """Generate visualizations for HDBSCAN cluster analysis"""
    # Create output directory if it doesn't exist
    os.makedirs(output_path, exist_ok=True)
    
    # Create a discrete colormap for cluster labels
    unique_clusters = sorted(list(set(cluster_labels)))
    # Use a qualitative colormap like tab20 for discrete categories
    colors = plt.cm.tab20(np.linspace(0, 1, len(unique_clusters) if len(unique_clusters) > 1 else 10))
    
    # Create a custom colormap that maps each cluster to a specific color
    cluster_colors = {}
    # Reserve black for noise points
    for i, cluster_id in enumerate(unique_clusters):
        if cluster_id == -1:
            cluster_colors[cluster_id] = 'black'
        else:
    ...
    for cluster_id in unique_clusters:
        ...
        x = [raw_centroids[i] for i in indices]
        y = [secondary_feature[i] for i in indices]
        
        # Use different marker for noise points
        if cluster_id == -1:
            plt.scatter(x, y, c='black', marker='x', alpha=0.7, s=40, label='Noise')
        else:
            plt.scatter(x, y, c=cluster_colors[cluster_id], marker='o', alpha=0.7, s=50, 
    ...
    if feature_matrix.shape[1] >= 2:
        ...
        # Use the same discrete color mapping for PCA visualization
        for cluster_id in unique_clusters:
            indices = np.where(cluster_labels == cluster_id)[0]
    ...
    # 3. HDBSCAN-specific: Condensed tree visualization
    if hasattr(clusterer, 'condensed_tree_'):
        plt.figure(figsize=(16, 10))
        # Count number of actual clusters (excluding noise)
        num_clusters = len(set(cluster_labels[cluster_labels >= 0]))
        ...
        plt.title(title)
        plt.tight_layout()
        plt.savefig(os.path.join(output_path, 'hdbscan_tree.png'), dpi=300)
        plt.close()
    
    # 4. Cluster sizes bar chart
    # Convert cluster labels to integers and handle -1 separately
    cluster_sizes = pd.Series(cluster_labels).value_counts().sort_index()
    
    plt.figure(figsize=(10, 6))
    colors = ['red' if idx == -1 else 'skyblue' for idx in cluster_sizes.index]
    cluster_sizes.plot(kind='bar', color=colors)
    
    plt.axhline(y=np.mean(cluster_sizes[cluster_sizes.index >= 0]), 
               color='blue', linestyle='--', label='Average Size (excl. noise)')
    
    # Format x-axis labels
    plt.xticks(range(len(cluster_sizes)), 
              [f"Noise" if idx == -1 else f"Cluster {idx}" for idx in cluster_sizes.index],
              rotation=45)
    ...
    # Extract raw features for plotting
    feature_dict = {
    ...
    # Box plots for features by cluster (excluding noise)
    non_noise_df = df_features[df_features['Cluster'] >= 0].copy()
    if len(non_noise_df) > 0:
        fig, axes = plt.subplots(1, 3, figsize=(18, 6))
        
        sns.boxplot(x='Cluster', y='Centroid', data=non_noise_df, ax=axes[0])
        axes[0].set_title('Aisle Centroid Distribution by Cluster')
        axes[0].grid(True, alpha=0.3)
        
        sns.boxplot(x='Cluster', y='Span', data=non_noise_df, ax=axes[1])
        axes[1].set_title('Aisle Span Distribution by Cluster')
        axes[1].grid(True, alpha=0.3)
        
        sns.boxplot(x='Cluster', y='DistinctAisles', data=non_noise_df, ax=axes[2])
